...
create_directory() {
	...
	# Some CWP builds harden permissions/attributes; chmod may be blocked (immutable/append-only).
	chmod "$mode" "$dir" 2>/dev/null || true
}

# Best-effort: temporarily remove immutable/append-only attributes on a set of paths,
# then restore them to their previous state.
# (Needed on newer CWP security postures where admin assets may be chattr +i.)
begin_attr_relax() {
	ATTR_SNAPSHOT_FILE=""
	...
	ATTR_SNAPSHOT_FILE=$(mktemp /tmp/${SCRIPT_NAME}.attrs.XXXXXX) || return 0

	# Snapshot attributes (directories and files, if present)
	for p in "$@"; do
		[[ -e "$p" ]] || continue
		# Format: attrs filename
		lsattr -d "$p" 2>/dev/null >>"$ATTR_SNAPSHOT_FILE" || true
		# Remove immutable and append-only flags
		chattr -i -a "$p" 2>/dev/null || true
	done
}

end_attr_relax() {
	if [[ -z "${ATTR_SNAPSHOT_FILE:-}" || ! -f "$ATTR_SNAPSHOT_FILE" ]]; then
		return 0
	fi
	if ! command_exists chattr; then
		rm -f "$ATTR_SNAPSHOT_FILE" || true
		return 0
	fi

	# Restore flags if they were set.
	# We only care about i/a; restore them when present in snapshot.
	while read -r attrs path; do
		[[ -n "${path:-}" && -e "$path" ]] || continue
		if [[ "$attrs" == *i* ]]; then
			chattr +i "$path" 2>/dev/null || true
		fi
		if [[ "$attrs" == *a* ]]; then
			chattr +a "$path" 2>/dev/null || true
		fi
	done <"$ATTR_SNAPSHOT_FILE"
...
install_cwp() {
	...
	# Newer CWP builds may set immutable/append-only attributes on admin asset dirs.
	# Temporarily relax attributes, perform install, then restore.
	begin_attr_relax \
		"/usr/local/cwpsrv/htdocs/admin" \
		"/usr/local/cwpsrv/htdocs/admin/design" \
		"/usr/local/cwpsrv/htdocs/admin/design/img" \
		"/usr/local/cwpsrv/htdocs/admin/design/js" \
		"/usr/local/cwpsrv/htdocs/resources/admin/include" \
		"/usr/local/cwpsrv/htdocs/resources/admin/modules"

	# Ensure dirs exist (do not fail if chmod is blocked)
	...
	install -m 755 "$tmp/${SCRIPT_NAME}.php" "/usr/local/cwpsrv/htdocs/resources/admin/modules/${SCRIPT_NAME}.php" || {
		end_attr_relax
		error_exit "Failed to install module PHP file to CWP modules directory"
	}

	# Copy icon/js
	install -m 644 "$tmp/${SCRIPT_NAME}.png" "/usr/local/cwpsrv/htdocs/admin/design/img/${SCRIPT_NAME}.png" || {
		end_attr_relax
		error_exit "Failed to install icon into CWP admin design/img (permission/attributes?)"
	}
	if [[ -s "$tmp/${SCRIPT_NAME}.js" ]]; then
		install -m 644 "$tmp/${SCRIPT_NAME}.js" "/usr/local/cwpsrv/htdocs/admin/design/js/${SCRIPT_NAME}.js" || true
	fi

	# Update IMH include (menu)
	install -m 644 "$tmp/imh-plugins.php" "/usr/local/cwpsrv/htdocs/resources/admin/include/imh-plugins.php" || true
	update_cwp_config || true

	# Restore immutable/append-only if they were present
	end_attr_relax
